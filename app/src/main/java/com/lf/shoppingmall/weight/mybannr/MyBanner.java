...
public class MyBanner extends RelativeLayout implements ViewPager.OnPageChangeListener {
    ...
    private int[] imags;
    private List<BannerVo> imageList;//图片集合
    ...
    private ArrayList<ImageView> getIvList() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (imageList == null) {
                ...
                ImageLoadUtils.loadingCommonPicBig(getContext(), imageList.get(i).getUrl(), iv);
            }
            final int index = i;
     ...
     */
    public void setImagesList(List<BannerVo> imageList, List<String> decsList, OnImageClickListener listener) {
...
//            }
    ...
    private void initVp() {
        ...
        if (decsList == null){
            tv_img_desc.setVisibility(GONE);
        }else {
            tv_img_desc.setVisibility(VISIBLE);
