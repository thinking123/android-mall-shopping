...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    /**
     * 当包裹的刷新目标为listView时，当listView滑动到底部时，自动加载更多数据
     * @param mListView
     */
    public void setmAutoLoading(ListView mListView){
        mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (scrollState == SCROLL_INDICATOR_END){
                    startLoadMore();
                }
            }
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                if (view.getLastVisiblePosition() != -1 && view.getLastVisiblePosition() == view.getCount() - 1){
                    startLoadMore();
                }
            }
        });
    ...
    public void closedLoadmore(int pageIndex, int pageSize, int total) {
